...
#include "neopixel.h"
  ...
  STM32_Pin_Info* PIN_MAPX = HAL_Pin_Map(); // Pointer required for highest access speed
  #define pinLO(_pin) (PIN_MAPX[_pin].gpio_peripheral->BSRRH = PIN_MAPX[_pin].gpio_pin)
  #define pinHI(_pin) (PIN_MAPX[_pin].gpio_peripheral->BSRRL = PIN_MAPX[_pin].gpio_pin)
#elif (PLATFORM_ID == 12) || (PLATFORM_ID == 13) || (PLATFORM_ID == 14) // Argon (12), Boron (13), Xenon (14)
  #include "nrf.h"
  #include "nrf_gpio.h"
  #include "pinmap_impl.h"
  NRF5x_Pin_Info* PIN_MAPX = HAL_Pin_Map();
  #define pinLO(_pin) (nrf_gpio_pin_clear(NRF_GPIO_PIN_MAP(PIN_MAPX[_pin].gpio_port, PIN_MAPX[_pin].gpio_pin)))
  #define pinHI(_pin) (nrf_gpio_pin_set(NRF_GPIO_PIN_MAP(PIN_MAPX[_pin].gpio_port, PIN_MAPX[_pin].gpio_pin)))
#else
...
//    pwm->INTEN |= (PWM_INTEN_SEQEND0_Enabled<<PWM_INTEN_SEQEND0_Pos);
    ...
    pwm->PSEL.OUT[0] = NRF_GPIO_PIN_MAP(PIN_MAPX[pin].gpio_port, PIN_MAPX[pin].gpio_pin);
  ...
  // ---------------------------------------------------------------------
    ...
    uint32_t pinMask = 1UL << NRF_GPIO_PIN_MAP(PIN_MAPX[pin].gpio_port, PIN_MAPX[pin].gpio_pin);
